...
- 自前実装のビルトインコマンドのみで動作（外部コマンドは実行しない）
- ビルトインファイル/ディレクトリ操作コマンド
- 現在のディレクトリ名をプロンプトに表示
- 包括的なエラーハンドリング
- cdコマンドのディレクトリ補完ヒント（簡易実装 / Tabキー未対応）
- **GUI版とCLI版の両方を提供**

## 前提条件（初回のみ）

ビルドには Rust ツールチェインと C リンカ（gcc）が必要です。

### 1. Rust ツールチェインのインストール

```bash
# rustup 経由で cargo / rustc をインストール
curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh -s -- -y

# 現在のシェルで cargo を使えるようにする（新しいターミナルでは自動で読み込まれる）
source "$HOME/.cargo/env"
```

### 2. C リンカ（build-essential）のインストール

Rust はリンクに C ツールチェインを使うため、`gcc` が無いとビルドに失敗します。
...
> **Note:** 初回ビルドは GUI 版の依存（egui など）のコンパイルに数分かかります。
> 変更を反映するための再ビルドも同じく `cargo build --release` を実行するだけです
> （変更したファイルだけが再コンパイルされるため通常は数秒で完了します）。
...
| `rm <file>` | ファイルを削除（ディレクトリには `-r` が必要） | `rm file.txt` |
...
このシェルは**上記の自前実装コマンドのみ**を実行します。
`echo` や `cat`、`grep` などの外部コマンドは実行できず、`command not found` になります。
